...
Dans cette arbre nous voyons qu'il y a une données mal classée. Dans un vrai scénario les nombre de données mal classé serait beacoup plus grand. Pour corrigé cette erreur, la prochaine étape est de calculer cette erreur. Il existe deux équations pour calculer cette erreur, soit, une pour la regression et une pour la classification.

Pour la regression on utilise :

$$
r_{im} = - \left[ \frac{\partial L(y_i, F(x_i))}{\partial F(x_i)} \right]_{F(x) = F_{m-1}(x)} \quad \text{for } i = 1, \dots, n.
$$

Cette equation représente
